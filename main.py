import pygame 
import os
pygame.font.init() #this initializes the font library from pygame
pygame.mixer.init() #this is the sound effect aspect of pygame
...
HEALTH_FONT = pygame.font.SysFont('comicsans', 40) #font for health score
WINNER_FONT = pygame.font.SysFont('comicsans', 100) #font for health score
...
def handle_bullets(red_bullets, yellow_bullets, red, yellow):
    for bullet in yellow_bullets:
        bullet.x += BULLET_VEL
        if red.colliderect(bullet): #colliderect() is from pygame to check for collision
    ...
    for bullet in red_bullets:
        bullet.x -= BULLET_VEL
        if yellow.colliderect(bullet): #colliderect() is from pygame to check for collision
        ...
        for event in pygame.event.get(): #this gets us the list of events and we're looping through them
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL  and len(yellow_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(yellow.x + yellow.width, yellow.y + yellow.height//2 - 2, 10, 5)#position, pos, width of bullet, height of bullet
                    yellow_bullets.append(bullet)
                    BULLET_FIRE_SOUND.play() #this plays the mp3

                if event.key == pygame.K_RCTRL and len(red_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(red.x, red.y + red.height//2 - 2, 10, 5)#position, pos, width of bullet, height of bullet
